...
with st.expander("Filtre", expanded=True):
    ...
    portfolio_label = c6.selectbox(
        "Vindu for porteføljeendring",
        list(portfolio_options.keys()),
        index=0,
        help="Bestem hvor langt tilbake vi ser når vi sammenligner aktive porteføljer (nå-vindu vs forrige vindu)."
    )
...
kpi_card("Total porteføljeverdi",
...
with colL:
    ...
    st.markdown('<div class="mm-title">Største porteføljer – meglere</div>', unsafe_allow_html=True)
    ...
    <div style="display:flex;gap:16px;padding:0 8px 8px 8px;color:#aeb4be;font-size:13px;">
      ...
      <div style="width:160px;text-align:right;">Porteføljeverdi</div>
    </div>
    """, unsafe_allow_html=True)
...
with colR:
    st.markdown(f'<div class="mm-card" style="{CARD_STYLE}">', unsafe_allow_html=True)
    st.markdown('<div class="mm-title">Største porteføljer – kontorer</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="mm-subtle">Antall meglere i utvalget: {brokers_total}</div>', unsafe_allow_html=True)
    st.markdown("""
    <div style="display:flex;gap:16px;padding:0 8px 8px 8px;color:#aeb4be;font-size:13px;">
      <div style="width:40px;">#</div>
...
st.markdown('<div class="mm-section-title">Porteføljeendringer</div>', unsafe_allow_html=True)
...
with cGrow:
    ...
    st.markdown('<div class="mm-title">Meglere med økt portefølje</div>', unsafe_allow_html=True)
    st.markdown(f'<div style="color:#aeb4be;font-size:13px;margin-bottom:10px;">Størst økning i aktiv portefølje ({portfolio_window_days} dager vs forrige {portfolio_window_days} dager)</div>', unsafe_allow_html=True)
    ...
    for i, row in grow.reset_index(drop=True).iterrows():
        ...
        chip = fmt_delta(row["delta_value"], row["delta_pct"])
        st.markdown(f"""
        <div class="mm-row vokser">
          ...
          <div class="mm-right">
            <div class="mm-value">{fmt_nok(total_now)}</div>
            {f'<div class="mm-chip up">↑ {chip}</div>' if chip else ''}
          </div>
        </div>
        """, unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
...
with cFall:
    st.markdown('<div class="mm-card">', unsafe_allow_html=True)
    st.markdown('<div class="mm-title">Meglere med redusert portefølje</div>', unsafe_allow_html=True)
    st.markdown(f'<div style="color:#aeb4be;font-size:13px;margin-bottom:10px;">Størst nedgang i aktiv portefølje ({portfolio_window_days} dager vs forrige {portfolio_window_days} dager)</div>', unsafe_allow_html=True)
    ...
    for i, row in fall.reset_index(drop=True).iterrows():
        ...
        <div class="mm-row faller">
          ...
          <div class="mm-right">
            <div class="mm-value">{fmt_nok(total_now)}</div>
            {f'<div class="mm-chip down">↓ {chip}</div>' if chip else ''}
          </div>
        </div>
        """, unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
